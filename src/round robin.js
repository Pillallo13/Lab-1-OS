// Simulación de un planificador Round Robin apropiativo con bloqueos
...
function crearProceso(id, tiempoTotal, bloqueos = []) {
    return {
      ...
      bloqueos,               // Array de { inicio, duracion } para bloqueos
      progreso: 0,            
      estado: 'listo',        
...
// -----> VISTA <------
  ...
  // -----> CONTROLADOR <------
  const timeQuantum = 2;           // Quantum de tiempo (unidades)
  let tiempo = 0;
  ...
  let quantumRestante = 0;         // Tiempo que le queda al proceso en su quantum
  ...
  // Comprobar bloqueos
    ...
    if (!ejecutando && colaListos.length > 0) {
      ...
      quantumRestante = timeQuantum;
      console.log(`  > ${ejecutando.id} comienza a ejecutarse (RR, quantum=${timeQuantum})`);
    }
  
    // Para todo proceso activo
    if (ejecutando) {
      // Avanza ejecución y consumo de quantum
      ejecutando.progreso++;
      quantumRestante--;
      console.log(
      ...
      // Verificar si un Quantum se agotó sin bloquearse ni terminar
      else if (quantumRestante <= 0) {
        ejecutando.estado = 'listo';
        colaListos.push(ejecutando);
        console.log(`Quantum agotado: ${ejecutando.id} vuelve a la cola`);
        ejecutando = null;
      }
    } else {
